...
void LocalMapping::CreateNewMapPoints()
{
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        // Step 5：通过极线约束限制匹配时的搜索范围，进行特征点匹配，但不检查旋转
        ...
        for(int ikp=0; ikp<nmatches; ikp++)
        {
            ...
            // 加1的目的是为了不是双目相机时，程序满足条件自动进入三角化恢复3D点，而不是使用双目恢复3D点
            float cosParallaxStereo = cosParallaxRays+1;  //一般相邻的关键帧应该不会有这么大的视角吧,90°多,不过感觉这个也不好说啊;（这句话是不对的）不过下面的程序中认为如果视差角的余弦值大于1是不正常的
            ...
            /*if(fabs(ratioDist-ratioOctave)>ratioFactor)
                continue;*/
            // ratioDist*ratioFactor < ratioOctave 或 ratioDist/ratioOctave > ratioFactor 表明尺度变化是连续的
